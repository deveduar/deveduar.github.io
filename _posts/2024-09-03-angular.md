...
- [x] Esbuild – *Angular 17* (`@if`, `@for`, funciones inject)
- [ ] Strapi CMS + Ionic
...
- `@Input()`  
	...
	- `[products]` en Home
- doble interpolación `{% raw %}{{ var }}{% endraw %}`
- imagen:
	```html
...
### 6.1 Tipos
- **Interpolación** `{% raw %}{{ variable }}{% endraw %}`
...
- Detalles:
	- `[]` recibe datos.
	- En template: `{% raw %}{{ datoEntrada }}{% endraw %}`.
...
- **Uso de plantillas**:
  ...
  - Binding de propiedades y eventos (`[src]`, `{% raw %}{{}}{% endraw %}`).
